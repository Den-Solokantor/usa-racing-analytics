      ...
      return `
        <div class="track-group${open}">
          <button type="button" class="track-group__toggle" aria-expanded="${expanded}">
            <span class="track-group__name"><span class="track-group__icon">${trackIcon(track)}</span> ${escapeHtml(track)}</span>
...
async function loadTips() {
  ...
  try {
    const res = await fetch('data/tips.json?t=' + Date.now());
    if (!res.ok) throw new Error('tips.json ' + res.status);
    const data = await res.json();
    ...
    if (meta) {
      ...
      if (data.stats) {
        if (data.stats.tips != null) parts.push(data.stats.tips + ' picks');
        if (data.stats.tracks != null) parts.push(data.stats.tracks + ' треков');
        if (data.stats.articles != null) parts.push(data.stats.articles + ' статей');
      }
      if (data.updated) {
        try {
          parts.push(
    ...
    if (!tips.length && !articles.length) {
      board.innerHTML = '<div class="loading-placeholder">Tips пока нет</div>';
      return;
    }

    const byTrack = {};
    tips.forEach((t) => {
      const k = t.track || 'Track';
    ...
    Object.keys(byTrack)
      ...
      .forEach((track) => {
        const rows = byTrack[track]
          .map((t) => {
            const race = t.race != null ? 'R' + t.race : '—';
            const post = t.post != null ? '#' + t.post : '';
            return (
              '<div class="tip-row">' +
              '<span class="tip-race">' +
              esc(race) +
    ...
    if (articles.length) {
      html +=
        '<div class="tip-articles"><h3>Статьи / обзоры</h3>' +
        articles
          .map(
            (a) =>
              '<a class="tip-article" href="' +
              esc(a.url || '#') +
    ...
    if (data.note) {
      html +=
        '<p class="section-desc" style="grid-column:1/-1;font-size:0.85rem;opacity:0.7">' +
        esc(data.note) +
        '</p>';
    }
    ...
    console.error(err);
    board.innerHTML =
      '<div class="loading-placeholder">Не удалось загрузить tips</div>';
  }
}
