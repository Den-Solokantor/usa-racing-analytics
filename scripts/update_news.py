...
FEEDS = [
    ...
    # BloodHorse RSS paths change; keep as optional
...
def relative_time(pub: str) -> str:
    ...
    try:
        ...
        mins = int((datetime.now(timezone.utc) - dt.astimezone(timezone.utc)).total_seconds() // 60)
        if mins < 1:
            return "только что"
        if mins < 60:
...
def translate_one(text: str) -> str:
    ...
    # 2) LibreTranslate public (best-effort)
    try:
...
def main() -> int:
    ...
    for i, it in enumerate(collected):
        ...
        time.sleep(0.8)
        summary_ru = translate_one(it.get("summary_en") or "") if it.get("summary_en") else ""
        time.sleep(0.8)
        items_out.append(
            {
                "title": title_ru,
